...
Original file is located at
    https://colab.research.google.com/drive/10nfvootHbVTbFuJTQEhBiQtdf0oNyeK2
"""
...
def plot_roc_curve():
    ...
    roc_stroke, ax = plt.subplots(figsize=(8, 8))
    plt.title('Stroke: Comparision of ROC Curves', fontsize=18)
...
def plot_shap_stroke():
    ...
    shap_stroke = plt.gcf()
    plt.title("Stroke: Top 5 Feature Contributions", fontsize=16)
    plt.tight_layout()
    return shap_stroke
...
# data.isnull().sum() ## bmi has 201 NAs
...
# Taining and test split
X_train, X_test, Y_train, Y_test = train_test_split(X,Y,test_size=0.2 ,random_state = 105)

# The percentage of positive cases (stroke) is : 4.87% # so use SMOTE
# Intializing our data sampling algorithm
smt = SMOTETomek(random_state=42)
X_train, Y_train = smt.fit_resample(X_train, Y_train)

## models
RF = RandomForestClassifier(n_estimators=100, max_depth = 20, random_state=47 , n_jobs = 3)
...
# call the function
results_df_stroke = get_results_df(Y_test, y_pred_RF, y_score_RF, y_pred_XG, y_score_XG, y_pred_LR, y_score_LR)
